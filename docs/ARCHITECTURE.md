...
(Story 2), streak/result persistence (Story 3), the optional hint (Story 6), run stats + the
copyable share summary (Story 9/10), and a first pass at Story 7's SFX (flag correct/wrong,
allow/block outcome, mute toggle) — Epics 1 and 4's core sharing loop are functionally covered.
Not yet implemented: the remaining injection techniques — homoglyph and split-payload — plus the
content-lint build script (Story 4), difficulty rotation (Story 5), the rest of Story 7's juice
plan (char-by-char typing animation on new lines, the matrix-lite falling-glyph win celebration,
and splitting the outcome sting into distinct allow-click/block-click/leak/win sounds per
`docs/DESIGN.md` instead of today's two-outcome mapping), and the archive of past puzzles
(Story 11).
